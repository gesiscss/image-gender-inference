...
def infering_gender(author_name, url_list, path_write):
    ...
    for url in url_list:
        ...
        #url = url.strip(']')
        #url = url.replace("'", '')
        apiurl = "https://apius.faceplusplus.com/v2/detection/detect?"
        param={"url":url,"attribute":"glass,pose,gender,age,race,smiling"}#"api_secret":API_SECRET,"api_key":API_KEY,
        apiurl+=urllib.urlencode(param)
        #apiurl += url
        apiurl += "&api_secret=" + API_SECRET + "&api_key=" + API_KEY
...
if __name__ == "__main__":
    ...
    with open(fileName, 'rb') as csvfile:
        spamreader = csv.reader(csvfile, skipinitialspace=True,delimiter=delimiter)
